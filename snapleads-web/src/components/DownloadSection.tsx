...
const DOWNLOAD_URL = "https://snapleads-releases.s3.us-east-1.amazonaws.com/SnapLeads+Setup+1.0.0.exe";
const VERSION = "1.0.0";
const VT_SCAN_URL = "https://www.virustotal.com/gui/url/956a3e5ffe0edb914fc0794682feb7606b13e39b613d034159d240b26995b2b7";
...
const bypassGuides: BypassGuide[] = [
  ...
  {
    id: "macos",
    title: "macOS Gatekeeper (Future Release)",
    icon: "\u{1F34E}",
    steps: [
      "macOS version coming soon \u2014 currently Windows only",
      "When available: Right-click the app then \"Open\" then \"Open\" again to bypass Gatekeeper",
      "Or: System Settings then Privacy & Security then \"Open Anyway\"",
    ],
  },
  {
    id: "linux",
    title: "Linux Installation (Future Release)",
    icon: "\u{1F427}",
    steps: [
      "Linux version coming soon \u2014 currently Windows only",
      "When available: Make the AppImage executable with chmod +x SnapLeads.AppImage",
      "Run with ./SnapLeads.AppImage",
      "If blocked: Check SELinux/AppArmor policies and add exception for SnapLeads",
    ],
  },
];
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className={`${t.card} rounded-xl p-8`}>
        <div className="flex items-start gap-5 mb-6">
          ...
          <div>
            <h3 className={`text-xl font-bold ${t.textPrimary}`}>SnapLeads Desktop App</h3>
            <p className={`${t.textSecondary} text-sm mt-1`}>Windows desktop application for license activation and lead extraction</p>
          </div>
        </div>
        ...
        <div className={`${t.cardInner} rounded-lg p-4 mb-6`}>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            ...
            <div>
              ...
              <span className={`${t.textPrimary} font-semibold`}>Windows 10/11</span>
            </div>
            <div>
              <span className={`${t.textMuted} block`}>Size</span>
              <span className={`${t.textPrimary} font-semibold`}>~85 MB</span>
            </div>
            <div>
              <span className={`${t.textMuted} block`}>Type</span>
              <span className={`${t.textPrimary} font-semibold`}>Installer (.exe)</span>
            </div>
          </div>
        </div>
      ...
      <div className={`${t.vtBadgeBg} border rounded-xl p-5`}>
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            <h4 className={`font-semibold ${t.vtBadgeText} mb-1`}>VirusTotal Verified &mdash; Clean &amp; Safe</h4>
            <p className={`text-sm ${t.textSecondary} mb-3`}>
              Our installer has been scanned by 70+ antivirus engines on VirusTotal.
      ...
      <div className={`${t.card} rounded-xl p-6`}>
        <h4 className={`${t.textPrimary} font-semibold mb-4 flex items-center gap-2`}>
          <Shield className="w-4 h-4" /> Installation &amp; Activation Steps
        </h4>
        <ol className={`space-y-3 text-sm ${t.textSecondary}`}>
          {[
            ...
            "Launch SnapLeads from your desktop or Start menu",
            "Enter your license key when prompted",
            "Click \"Activate\" \u2014 your license will be verified online",
            "Start extracting leads! Your license is now active",
          ].map((step, i) => (
      ...
      <div className={`${t.card} rounded-xl p-6`}>
        <div className="flex items-center gap-2 mb-2">
          ...
          <h4 className={`${t.textPrimary} font-semibold`}>Bypass Download &amp; Install Restrictions</h4>
        </div>
        <p className={`${t.textSecondary} text-sm mb-4`}>
          Some systems may show security warnings for new software. Here is how to fix them on every platform:
        </p>
      ...
      <div className={`${t.warnBg} border rounded-xl p-4`}>
        <p className={`text-sm ${t.warnText}`}>
          ...
          <a href={VT_SCAN_URL} target="_blank" rel="noopener noreferrer" className="underline font-medium hover:no-underline">VirusTotal</a>.
        </p>
      </div>
    </div>
  );
