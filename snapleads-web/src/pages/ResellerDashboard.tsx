        ...
        <div className={`flex gap-1 ${t.tabBg} rounded-xl p-1 w-fit`}>
          {tabs.map(([id, label, Icon]) => (
            <button key={id} onClick={() => setTab(id)}
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition ${tab === id ? "bg-purple-600 text-white shadow" : t.tabInactive}`}>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className={`${t.card} rounded-xl p-5`}>
                <div className="flex items-center gap-2 mb-3"><TrendingUp className="w-4 h-4 text-emerald-400" /><span className={`${t.textSecondary} text-sm`}>Revenue</span></div>
          ...
          <div className="space-y-4">
            <div className="relative max-w-md">
              ...
              <input type="text" value={searchQuery} onChange={(e) => { setSearchQuery(e.target.value); setKeysPage(1); }}
                placeholder="Search keys..." className={`w-full pl-10 pr-4 py-2.5 ${t.selectInput} border rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500/50 text-sm`} />
            </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className={`border-b ${t.border}`}>
                    ...
                    <th className={`text-left px-4 py-3 text-xs font-medium ${t.textSecondary} uppercase`}>Copy</th>
                  </tr>
                </thead>
                <tbody>
                  {keys.map((k) => (
                    <tr key={k.id} className={`border-b ${t.borderLight} ${t.rowHover}`}>
                      <td className="px-4 py-3"><code className={`text-sm ${t.codeText} font-mono`}>{k.key}</code></td>
        ...
        {tab === "generate" && (
          <div className="max-w-2xl space-y-6">
            <form onSubmit={handleGenerate} className={`${t.card} rounded-xl p-6 space-y-5`}>
              <h3 className={`text-lg font-semibold ${t.textPrimary} flex items-center gap-2`}><Plus className="w-5 h-5 text-purple-400" /> Generate Customer Keys</h3>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={genPlan} onChange={(e) => setGenPlan(e.target.value)}
                    className={`w-full px-3 py-2.5 ${t.input} border rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500/50`}>
              ...
              <div><label className={`block text-sm font-medium ${t.label} mb-1.5`}>Quantity (1-50)</label>
                <input type="number" min={1} max={50} value={genQty} onChange={(e) => setGenQty(Number(e.target.value))}
                  className={`w-full px-3 py-2.5 ${t.input} border rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500/50`} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div><label className={`block text-sm font-medium ${t.label} mb-1.5`}>Customer Name</label>
                  <input type="text" value={genName} onChange={(e) => setGenName(e.target.value)}
                    className={`w-full px-3 py-2.5 ${t.input} border rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500/50`} placeholder="Customer name" /></div>
                <div><label className={`block text-sm font-medium ${t.label} mb-1.5`}>Customer Email</label>
                  <input type="email" value={genEmail} onChange={(e) => setGenEmail(e.target.value)}
                    className={`w-full px-3 py-2.5 ${t.input} border rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500/50`} placeholder="customer@email.com" /></div>
              </div>
